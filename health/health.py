...
class Appointment(ModelSQL, ModelView):
    ...
    queue_number = fields.Char('Queue Number')
    ...
    def checked_in(cls, appointments):
        for appointment in appointments:
            # Kirim permintaan GET ke API Laravel
            if appointment.queue_number:
                try:
                    response = requests.get(f'http://103.67.78.79/antrian/update_dokter/{appointment.queue_number}')
                    if response.status_code == 200:
                        cls.write(appointments, {'state': 'checked_in'}) 
                        print(f'Successfully updated queue for queue number {appointment.queue_number}')
                    else:
                        print(f'Failed to update queue for queue number {appointment.queue_number}. Status code: {response.status_code}')
                except requests.exceptions.RequestException as e:
                    print(f'Error updating queue for queue number {appointment.queue_number}: {e}') 
    ...
    def create(cls, vlist):
        ...
        for values in vlist:
            ...
            if queue_number:
                try:
                    response = requests.get(f'http://103.67.78.79/antrian/delete_pendaftaran/{queue_number}')
                    if response.status_code == 200:
                        print(f'Successfully deleted registration for queue number {queue_number}')
                    else:
                        print(f'Failed to delete registration for queue number {queue_number}. Status code: {response.status_code}')
                except requests.exceptions.RequestException as e:
                    print(f'Error deleting registration for queue number {queue_number}: {e}') 
...
class PatientEvaluation(ModelSQL, ModelView, MultiValueMixin):
    ...
    def end_evaluation(cls, evaluations):
        ...
        for evaluation in evaluations:
            # Kirim permintaan GET ke API Laravel
            if evaluation.queue_number:
                try:
                    response = requests.get(f'http://103.67.78.79/antrian/close_dokter/{evaluation.queue_number}')
                    if response.status_code == 200:
                        cls.write(evaluations, {'state': 'done','signed_by': signing_hp,'evaluation_endtime': datetime.now()})
                        print(f'Successfully close evaluation')
                    else:
                        print(f'Failed to close evaluation. Status code: {response.status_code}')
                except requests.exceptions.RequestException as e:
                    print(f'Error updaticlose evaluation: {e}')
