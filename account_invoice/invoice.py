...
class Invoice(Workflow, ModelSQL, ModelView, TaxableMixin):
    ...
    def post(cls, invoices):
        ...
        for invoice in invoices:
            # Ambil semua baris invoice yang terkait
            lines = Line.search([('invoice', '=', invoice.id)])
            
            # Periksa apakah ada produk dengan is_medical_supply atau is_medicament = True
            post_to_api = False
            for line in lines:
                product = Product(line.product.id)
                if product.is_medical_supply or product.is_medicament:
                    post_to_api = True
                    break
            
            # Kirim POST ke API jika diperlukan
            if post_to_api:
                if invoice.queue_number:
                    try:
                        response = requests.get(
                            f'http://103.67.78.79/antrian/delete_pembayaran/{invoice.queue_number}'
                        )
                        if response.status_code == 200:
                            print(f'Successfully deleted queue for queue number {invoice.queue_number}')
                        else:
                            print(f'Failed to delete queue for queue number {invoice.queue_number}. Status code: {response.status_code}')
                    except requests.exceptions.RequestException as e:
                        print(f'Error deleting queue for queue number {invoice.queue_number}: {e}')
            
            # Lakukan proses posting invoice
            cls._post([invoice])
